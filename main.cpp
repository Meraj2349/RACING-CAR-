...
int main()
{
	...
	//image lode 
	sf::Texture gamefront,background, racer, obs1, obs2, obs3, obs4, gameover;
	// font
	Font myfont;
	myfont.loadFromFile("xirod.ttf");
	//lode into code ->image 
	if (!gamefront.loadFromFile("gamefront.jpg"))
	{
		cout << "gamefront loding error!";
	}
	...
	sf::Sprite  Gamefront(gamefront), Background(background), Background1(background), Racer(racer), Obs1(obs1), Obs2(obs2);
	double RacerX, RacerY, Obs1X, Obs1Y, Obs2X, Obs2Y;
	window.draw(Gamefront);
	...
	//loop for game 
			while (window.isOpen())
			{
				...
				if (start)
				{
					...
					window.draw(text);
					if (Keyboard::isKeyPressed(Keyboard::Enter))	start = false;
